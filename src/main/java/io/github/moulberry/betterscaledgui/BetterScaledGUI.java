...
public class BetterScaledGUI {
    ...
    SimpleCommand setHotbarScalingCommand = new SimpleCommand("hotbarscale", new SimpleCommand.ProcessCommandRunnable() {
        @Override
        public void processCommand(ICommandSender sender, String[] args) {
            if(args.length > 1) {
                sender.addChatMessage(new ChatComponentText(PREFIX+EnumChatFormatting.RED+"Too many arguments. Use '/hotbarscale help' for assistance"));
                return;
            }
            if(args.length == 0 ||
                    args[0].equalsIgnoreCase("help")) {
                sender.addChatMessage(new ChatComponentText(PREFIX+EnumChatFormatting.YELLOW+"           Help"));
                sender.addChatMessage(new ChatComponentText(PREFIX+EnumChatFormatting.YELLOW+"Usage: /hotbarscale [scaling]"));
                sender.addChatMessage(new ChatComponentText(PREFIX+EnumChatFormatting.YELLOW+"Scaling may be a number between 1-5, or"));
                sender.addChatMessage(new ChatComponentText(PREFIX+EnumChatFormatting.YELLOW+"small/normal/large/auto"));
                sender.addChatMessage(new ChatComponentText(PREFIX+EnumChatFormatting.YELLOW+"Use '/hotbarscale off' to disable scaling"));
                return;
            }
            if(args[0].equalsIgnoreCase("off") ||
                    args[0].equalsIgnoreCase("none")) {
                ScaledResolutionOverride.setHotbarScaleOverride(-1);
                sender.addChatMessage(new ChatComponentText(PREFIX+EnumChatFormatting.GREEN+"Disabled hotbar scaling"));
                saveConfig();
                return;
            }
            int scaling;
            if(args[0].equalsIgnoreCase("small") || args[0].equalsIgnoreCase("s")) {
                ...
                try {
                    ...
                    sender.addChatMessage(new ChatComponentText(
                            PREFIX+EnumChatFormatting.RED+"Invalid scaling identifier. Use '/hotbarscale help' for assistance"));
                    return;
                }
            }
            if(scaling < 1) {
            ...
            sender.addChatMessage(new ChatComponentText(PREFIX+EnumChatFormatting.GREEN+"Set hotbar scaling to: " + scaling));
            ScaledResolutionOverride.setHotbarScaleOverride(scaling);
            saveConfig();
        }
    });
    ...
    public void preinit(FMLPreInitializationEvent event) {
        MinecraftForge.EVENT_BUS.register(new HotbarScaler());

        ClientCommandHandler.instance.registerCommand(setScalingCommand);
        ClientCommandHandler.instance.registerCommand(setHotbarScalingCommand);
    ...
    private void saveConfig() {
        if(configFile != null) {
            try {
                ...
                object.addProperty("hotbarscale", ScaledResolutionOverride.getHotbarScaleOverride());
        ...
        if(configFile != null) {
            try {
                try(BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream(configFile), StandardCharsets.UTF_8))) {
                    ...
                    ScaledResolutionOverride.setHotbarScaleOverride(json.get("hotbarscale").getAsInt());
                }
            } catch(Exception e) {
